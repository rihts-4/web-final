...
export function HashtagList({ title = "Related tags", compact = false, onTagClick }: HashtagListProps) {
  return (
    <div>
      {title && (
        <h3
        ...
        <div className="flex flex-wrap gap-2">
          {TAGS.map((tag) => (
            <button
              ...
              style={{
                background: tag.trending ? "rgba(107,143,94,0.15)" : "rgba(42,42,37,0.06)",
                color: tag.trending ? "#6B8F5E" : "#5A5A52",
                fontWeight: 600,
                border: tag.trending ? "1px solid rgba(107,143,94,0.3)" : "1px solid rgba(42,42,37,0.1)",
              }}
            >
              <span>{tag.emoji}</span>
              <span>#{tag.name}</span>
              {tag.trending && (
                <span className="text-[10px]" style={{ color: "#6B8F5E", opacity: 0.7 }}>
                  ↑
                </span>
              )}
            </button>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {tag.trending && (
                    <span
                      ...
                      style={{ background: "rgba(107,143,94,0.12)", color: "#6B8F5E", fontWeight: 700 }}
                    >
                      Trending
                    </span>
                  )}
                </div>
                <span className="text-muted-foreground text-[12px]">{fmt(tag.count)} thoughts</span>
              </div>
              <span className="text-muted-foreground text-[12px] flex-shrink-0">#{i + 1}</span>
            </button>
          ))}
        </div>
      )}
