  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "#FDFAF4", border: "1px solid rgba(42,42,37,0.1)", fontFamily: "'Nunito', sans-serif" }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-5 pb-3">
          <span className="text-[12px] uppercase tracking-widest" style={{ color: "#6B8F5E", fontWeight: 700 }}>
        ...
        <div className="px-6 mb-5">
          <div className="h-1 rounded-full" style={{ background: "rgba(42,42,37,0.1)" }}>
            <div
              className="h-full rounded-full transition-all duration-500"
              style={{ width: `${((step + 1) / STEPS.length) * 100}%`, background: "#6B8F5E" }}
            />
          </div>
        </div>
        ...
        <div className="px-6 pb-2 min-h-[220px]">
          ...
          <h2 className="text-foreground text-[22px] mb-3" style={{ fontWeight: 800 }}>{current.title}</h2>
          <p className="text-[15px] leading-relaxed" style={{ color: "#5A5A52" }}>{current.body}</p>

          {/* Swipe visual */}
          {current.visual === "swipe" && (
            <div className="mt-4 flex items-center gap-4 justify-center">
              <div className="flex flex-col items-center gap-1">
                <div className="flex items-center gap-2 px-4 py-2 rounded-xl border-2 text-[13px]" style={{ borderColor: "#C0453A", color: "#C0453A", fontWeight: 700 }}>
                  ← Swipe left
                </div>
                <span className="text-[12px] text-muted-foreground">Open replies</span>
              </div>
              <div
                className="w-12 h-16 rounded-xl shadow-md flex-shrink-0"
                style={{ background: "linear-gradient(135deg, #FDFAF4, #EAE5D8)", border: "1.5px solid rgba(42,42,37,0.12)" }}
              />
              <div className="flex flex-col items-center gap-1">
                <div className="flex items-center gap-2 px-4 py-2 rounded-xl border-2 text-[13px]" style={{ borderColor: "#6B8F5E", color: "#6B8F5E", fontWeight: 700 }}>
                  Swipe right →
                </div>
          ...
          {current.highlight && (
            <div
              ...
              style={{ background: "rgba(107,143,94,0.1)", borderLeft: "3px solid #6B8F5E", color: "#4A6B3F", fontWeight: 600 }}
            >
              {current.highlight}
            </div>
          )}
        ...
        <div className="flex items-center justify-between px-6 py-5">
            ...
            {STEPS.map((_, i) => (
              <button
                ...
                style={{
                  width: i === step ? 20 : 7,
                  height: 7,
                  background: i === step ? "#6B8F5E" : i < step ? "rgba(107,143,94,0.4)" : "rgba(42,42,37,0.15)",
                }}
              />
            ))}
          </div>
          ...
          {isLast ? (
            <button
              ...
              style={{ background: "#6B8F5E", color: "#FDFAF4", fontWeight: 700, boxShadow: "0 4px 12px rgba(107,143,94,0.3)" }}
            >
              Enter Grove 🌿
            </button>
          ) : (
