...
export function ComposeModal({ onClose, onPost, currentUser }: ComposeModalProps) {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "#FDFAF4", border: "1px solid rgba(42,42,37,0.1)" }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <div className="flex items-center gap-2" style={{ color: "#6B8F5E" }}>
            <PenLine size={18} />
            <span className="text-[15px]" style={{ fontWeight: 700, color: "#2A2A25" }}>New Thought</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex gap-3 px-5 pb-4">
          <img
            ...
            style={{ outline: "2px solid rgba(107,143,94,0.3)", outlineOffset: 2 }}
          />
          <div className="flex-1">
            <textarea
              ...
              style={{ color: "#2A2A25", fontFamily: "'Nunito', sans-serif", fontWeight: 400 }}
            />
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-3" style={{ borderTop: "1px solid rgba(42,42,37,0.08)" }}>
          <div className="flex items-center gap-1" style={{ color: "#6B8F5E" }}>
            {[Image, Smile, Volume2, BarChart2].map((Icon, i) => (
              <button key={i} className="p-2 rounded-xl transition-colors hover:bg-secondary">
                <Icon size={17} />
              </button>
            ))}
          </div>
          <div className="flex items-center gap-3">
            {content.length > 0 && (
              <span className="text-[12px]" style={{ color: remaining < 30 ? (overLimit ? "#C0453A" : "#B5A040") : "#B5B0A4", fontWeight: 600 }}>
                {remaining}
              </span>
            )}
            <button
              ...
              style={{
                ...
                color: "#FDFAF4",
                fontWeight: 700,
                boxShadow: content.trim() ? "0 4px 14px rgba(107,143,94,0.4)" : "none",
              }}
            >
              Plant it 🌱
            </button>
