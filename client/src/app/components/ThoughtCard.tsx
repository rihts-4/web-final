...
import { Heart, Repeat2, Bookmark, MessageCircle, Play, Pause, Volume2 } from "lucide-react";
...
function AudioPlayer({ label, duration }: { label: string; duration: string }) {
  ...
  const bars = [3, 5, 4, 7, 3, 6, 4, 8, 5, 3, 6, 4, 7, 3, 5, 6, 4, 3, 7, 5, 4, 6, 3, 5];
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[12px] mb-1.5 truncate" style={{ color: "#5A5A52" }}>{label}</p>
            ...
            {bars.map((h, i) => (
              <div
                ...
                style={{
                  ...
                  background: (i / bars.length) * 100 <= progress ? "#6B8F5E" : "rgba(42,42,37,0.12)",
                }}
              />
            ))}
          </div>
      ...
      <div className="flex items-center gap-1 flex-shrink-0" style={{ color: "#B5B0A4" }}>
...
export function ThoughtCard({
  post, onLike, onRepost, onBookmark, onReply, onSwipedAway, style, isTop,
}: ThoughtCardProps) {
  ...
  const [gesture, setGesture] = useState<"like" | "repost" | null>(null);
  ...
  const repostAlpha = gesture === "repost" ? Math.min(1, (-dragX - 30) / 60) : 0;
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          />
          <div className="flex-1 min-w-0">
            <span className="block text-foreground text-[15px] truncate" style={{ fontWeight: 700 }}>
              {post.user.name}
            </span>
        ...
        {/* Image */}
        {post.image && (
          <div className="rounded-2xl overflow-hidden mb-4" style={{ maxHeight: 200 }}>
            <img src={post.image} alt="" className="w-full object-cover" style={{ maxHeight: 200 }} />
          </div>
        )}
        ...
        >
          ...
          <ActionBtn
            ...
            icon={<Heart size={19} strokeWidth={2.4} fill={post.liked ? "currentColor" : "none"} />}
          ...
          <ActionBtn
            ...
            icon={<Bookmark size={19} strokeWidth={2.4} fill={post.bookmarked ? "currentColor" : "none"} />}
            label=""
          ...
          <span
            ...
            style={{ color: "#D4CFC6", fontWeight: 600, fontFamily: "'Nunito', sans-serif" }}
          >
            ← swipe →
          </span>
        </div>
...
export function UserAvatar({ avatar, name, size = 40, verified, onClick }: UserAvatarProps) {
  ...
  return (
    ...
    >
      <img
        ...
        style={{ outline: "2.5px solid rgba(107,143,94,0.25)", outlineOffset: 2 }}
      />
      {verified && (
        <div
        ...
        >
          <svg viewBox="0 0 10 10" width={badge * 0.58} height={badge * 0.58} fill="none">
            <path
              d="M2 5l2 2 4-4"
              stroke="white"
              strokeWidth="2"
...
/* ── ActionBtn ────────────────────────────────────────── */
function ActionBtn({
  onClick, icon, label, activeColor, tooltip,
}: {
  ...
  return (
    ...
    >
      {icon}
      {label && (
        <span className="text-[13px]" style={{ fontWeight: 600 }}>{label}</span>
      )}
    </button>
  );
}
