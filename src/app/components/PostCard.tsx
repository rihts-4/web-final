...
import { MessageCircle, Repeat2, Share, Bookmark, MoreHorizontal, Zap } from "lucide-react";
...
export function PostCard({ post, onLike, onRepost, onBookmark, onReply }: PostCardProps) {
  ...
  return (
    <article
      ...
      style={{ backgroundColor: hovered ? "rgba(139,92,246,0.04)" : "transparent" }}
    >
      ...
      <div className="relative flex-shrink-0">
        ...
        />
        {post.user.verified && (
          <div className="absolute -bottom-1 -right-1 w-4.5 h-4.5 rounded-full bg-primary flex items-center justify-center" style={{ width: 18, height: 18 }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between gap-2 mb-1">
          <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
            <span className="text-foreground text-[15px] truncate" style={{ fontWeight: 700 }}>
              {post.user.name}
            </span>
            <span className="text-muted-foreground text-[14px] truncate">@{post.user.handle}</span>
            <span className="text-muted-foreground text-[14px]">·</span>
            <span className="text-muted-foreground text-[14px] flex-shrink-0">{post.timestamp}</span>
          </div>
          <button
...
function ActionBtn({
  ...
  active?: boolean;
  activeColor?: string;
  hoverColor: "primary" | "emerald" | "amber";
}) {
  const hoverClasses = {
