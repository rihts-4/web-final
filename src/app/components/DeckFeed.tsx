  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center px-8">
        <div className="w-16 h-16 rounded-full mb-4 flex items-center justify-center" style={{ background: "rgba(107,143,94,0.15)" }}>
          <span className="text-2xl">🌱</span>
        </div>
        <h2 className="text-foreground text-[22px] mb-2" style={{ fontWeight: 800 }}>All caught up</h2>
        <p className="text-muted-foreground text-[15px]">You've read every thought. Come back later.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
        ...
        {posts.map((_, i) => (
          <div
            ...
            style={{
              ...
              height: 6,
              background: i === topIndex ? "#6B8F5E" : i < topIndex ? "rgba(107,143,94,0.3)" : "rgba(42,42,37,0.15)",
            }}
          />
        ))}
      </div>
      ...
      <div className="flex items-center justify-between px-6 pb-5 flex-shrink-0">
        <button
          ...
          style={{ borderColor: "rgba(42,42,37,0.15)", background: "#FDFAF4", fontWeight: 600, fontSize: 13, color: "#2A2A25" }}
        >
          ← Back
        </button>

        <div className="text-center">
          <p className="text-muted-foreground text-[12px]" style={{ fontWeight: 500 }}>
            {topIndex + 1} of {posts.length}
          </p>
          <p className="text-[11px] text-muted-foreground/60">swipe to interact</p>
        </div>
